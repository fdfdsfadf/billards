...
fg_api.prototype.adsModule = function () {
  ...
  module.prototype.init = function () {
    ...
    switch (M.provider) {
      ...
      case "dfp":
        ...
        if (
          self.config.ads.mcm_network_code &&
          detection.is.iframe &&
  ...
  module.prototype.showVideoAd = function (
    callback,
    force,
    isAutoplay = false
  ) {
        ...
        if (detection.is.ios)
          self.click2play.showOverlay(
            function () {
              ...
              if (self.interstitial && self.interstitial.isEnabled()) {
                self.log(
                  "ads module: attempt to load web interstitial instead of video ad"
                );
                self.interstitial.showAd(continueToGame);
              } else {
  ...
  module.prototype.showIMA = function (
    callback,
    force,
    ...
    function sendAdRequest(custParamsQS, vastSizeString, requestType, adUnit) {
      ...
      try {
        ...
        if (Math.random() < 0.01) {
          let stroerAdTagUrl =
            "https://vh.adscale.de/vah?sid=e6b72f77-feff-40a9-80d9-6a7dec37bbf9&ref=%%REFERRER_URL%%&gdpr=%%GDPR%%&gdpr_consent=%%GDPR_CONSENT_STRING%%&bust=%%CACHEBUSTER%%";
    ...
    if (self.apstag && self.apstag.isReadyToFetch()) {
      ...
      if (supportedSizes.length == 0) supportedSizes = [{ w: 640, h: 480 }];

      // get APS video slot ID for biggest size
      supportedSizes = supportedSizes.slice(0, 1);
      let apsSlotID = self.apstag.getVideoSlotID(
        supportedSizes[0].w,
        supportedSizes[0].h
      );
      ...
      self.apstag.fetchVideoBids(apsSlotID).then(
        function (bids) {
        ...
        function () {
          self.log(
            "video ads: failed to prefetch bids, will send std request instead"
          );
          sendAdRequest();
        }
      );
  ...
  module.prototype.getAdTagUrl = function (
    ...
    if (detection.is.iframe) {
      if (M.mcm_required) {
        ...
        else descriptionUrl = self.getShortLink();
    ...
    M.dfp_custom_params = {
      ...
      arc:
        M.adRequestCount > 20
          ...
          : "0" + M.adRequestCount,
      aic:
        M.adImpressionCount > 20
          ? "gt20"
          : M.adImpressionCount >= 10
          ? "" + M.adImpressionCount
    ...
    if (M.dfp_custom_params) {
      ...
      if (typeof customParamsQueryString == "string") {
        if (qs.length > 0) qs += customParamsQueryString;
        else qs = customParamsQueryString.substring(1);
      }
  ...
  module.prototype.getSiteUrl = function () {
    ...
    if (detection.is.iframe) {
      ...
      else siteUrl = self.getShortLink();
    } else {
      siteUrl = self.getShortLink();
    }
...
fg_api.prototype.rewardedadsModule = function () {
  ...
  module.prototype.init = function () {
    ...
    function setupGPTEventListeners() {
      ...
      window.googletag
        .pubads()
        .addEventListener("rewardedSlotClosed", function (evt) {
          if (evt.slot !== M.rewardedAd.slot) {
...
fg_api.prototype.interstitialModule = function () {
  ...
  module.prototype.init = function () {
    ...
    if (urlParams.fg_env && urlParams.fg_env == "dev") {
      ...
      M.adUnitPath = "/" + M.GAM_NETWORK_CODE + "/test-adx-gmi";
    } else {
      M.adUnitPath =
        "/" +
        M.GAM_NETWORK_CODE +
  ...
  module.prototype.setupListeners = function () {
    ...
    window.googletag
      ...
      .addEventListener("slotRenderEnded", function (evt) {
        ...
        if (evt.isEmpty) {
          ...
          if (M.showAdInProgress()) M.resumeGame();
          return;
        }
        M.adDidLoad = true;
        M.numAdFills += 1;
    ...
    window.googletag.pubads().addEventListener("slotRequested", function (evt) {
      if (!M.hasSlot(evt.slot)) {
    ...
    window.googletag
      .pubads()
      .addEventListener("gameManualInterstitialSlotReady", function (evt) {
        if (!M.hasSlot(evt.slot)) {
          self.log(
            "interstitial: ignored gameManualInterstitialSlotReady event"
          );
          return;
        }
        ...
        if (M.showAdInProgress()) M.displayCreative();
      });

    window.googletag
      .pubads()
      .addEventListener("gameManualInterstitialSlotClosed", function (evt) {
        if (!M.hasSlot(evt.slot)) {
          self.log(
            "interstitial: ignored gameManualInterstitialSlotClosed event"
          );
          return;
        }
  ...
  module.prototype.clearSlots = function () {
    ...
    window.googletag.cmd.push(function () {
      if (M.oldSlots.length > 0) window.googletag.destroySlots(M.oldSlots);
    });
  };
  ...
  module.prototype.displayCreative = function () {
    ...
    if (M.slotReadyEvent) {
      ...
      self.log(
        "interstitial: no slot ready event, interstitial failed, resume"
      );
      M.resumeGame();
    }
  };
  ...
  module.prototype.requestAd = function () {
    ...
    window.googletag.cmd.push(function () {
      ...
      M.currentSlot = window.googletag.defineOutOfPageSlot(
        M.adUnitPath,
      ...
      if (M.currentSlot) {
        ...
        // no ad slot created
        self.log(
          "interstitial: slot failed - page or device does not support interstitials"
        );
  ...
  module.prototype.hasCooledDown = function () {
    return (
      self.now() - M.COOLDOWN_SEC * 1000 > M.lastAdRequest ||
      M.preloadState == M.preloadStates.FINISHED
    );
  };
...
fg_api.prototype.bannerModule = function () {
  ...
  module.prototype.init = function () {
    ...
    M.TYPES.forEach(function (type) {
      ...
      M.containers[type] =
        M.getContainerElement(type) || M.createContainerElement(type);
    });
  ...
  module.prototype.parseBannerConfig = function () {
    ...
    if (
      typeof self.config.gameParams.banner == "object" &&
      self.config.gameParams.banner.enabled
    ) {
  ...
  module.prototype.isEnabled = function () {
    return (
      M.forceBannerOn ||
      (self.hasFeature("ads") &&
        (!!self.config.gameParams.banner_enabled ||
          (self.config.gameParams.banner &&
            self.config.gameParams.banner.enabled)) &&
        !self.config.ads.off &&
        window.famobi_env != "vp" &&
        self.config.gameParams.ad_type != "off" &&
  ...
  module.prototype.getContainerID = function (type, depth) {
    return (
      (depth == "inner" ? M.INNER_CONTAINER_PREFIX : M.OUTER_CONTAINER_PREFIX) +
      "-" +
      type
    );
  };
  ...
  module.prototype.createContainerElement = function (type) {
    ...
    var containerStyle =
      "position: fixed; display: flex; z-index: 999; overflow: visible; user-select: none; ";
    ...
    if (M.CSS_FADE_IN) {
      var fadeCSS = "";
      if (type == "bottom") fadeCSS += "transform: translateY(100%); ";
      if (type == "top") fadeCSS += "transform: translateY(-100%); ";
      if (type == "left") fadeCSS += "transform: translateX(-100%); ";
      if (type == "right") fadeCSS += "transform: translateX(100%); ";
      ...
      if (isFullContainer) containerStyle += fadeCSS;
      else innerContainerStyle += fadeCSS;
    }
          ...
          if (M.slots[type]) {
            ...
            if (
              ...
              if (self.apstag && self.apstag.isReadyToFetch()) {
                ...
                self.apstag
                  .fetchAndApplyDisplayBids(containerID, adUnitPath, sizesArray)
                  .then(
                    function (bids) {
                    ...
                    function () {
                      self.log(
                        "banner: failed to prefetch aps bids, will send std request instead"
                      );
                    }
                  )
                  .finally(function () {
  ...
  module.prototype.checkViewportSupport = function () {
    ...
    M.TYPES.forEach(function (type) {
      ...
      if (
        (M.config[type].max_ratio != null &&
          ratio > M.config[type].max_ratio) ||
      ...
      if (
        (M.config[type].min_width != null &&
          size.w < M.config[type].min_width) ||
        (M.config[type].min_height != null &&
          size.h < M.config[type].min_height)
      ) {
        return;
      }
    ...
    M.TYPES.forEach(function (type) {
      if (
        oldSupport[type] !== M.support[type] ||
        oldSizes[type].length !== M.sizes[type].length
      )
        hasChanged = true;
    });
    if (hasChanged) self.adapters.run("banner", "placementChanged");
  };
  ...
  module.prototype.getSlotType = function (slot) {
    var slotType = null;
    M.TYPES.forEach(function (type) {
      if (M.slots[type] === slot) slotType = type;
    });
    return slotType;
  };
...
fg_api.prototype.apstagModule = function () {
  ...
  module.prototype.fetchVideoBids = function (slotID) {
    return new Promise(function (resolve, reject) {
      ...
      let requestSlotID =
        typeof slotID !== "undefined" ? slotID : M.DEFAULT_SLOT_ID;
  ...
  module.prototype.fetchAndApplyDisplayBids = function (
    slotElementID,
    adUnitPath,
    sizesArray
  ) {
    return new Promise(function (resolve, reject) {
      ...
      if (!slotElementID) {
        self.log(
          "apstag: cannot fetch display bids, no slot element id provided"
        );
        return reject();
      }
      ...
      // Request Bids
      self.log(
        "apstag: requesting bids for display slot",
        slotElementID,
        ", with ad unit path",
        adUnitPath,
        "and sizes",
...
fg_api.prototype.adaptersModule = function () {
  ...
  function module() {
    ...
    this.adapters = {
      ...
      banner: {
        placementChanged: this.createAdapter({
          queueCalls: true,
          queueLength: 1,
          runOnce: false,
        }),
...
fg_api.prototype.consentModule = function () {
  ...
  function module() {
    ...
    if (self.debug && !this.cookieproPreview) this.cookieproScriptID += "-test";
  ...
  module.prototype.setupTcfApiListener = function () {
    ...
    __tcfapi("addEventListener", 2, function (tcData, success) {
      self.log(
        "consent: TCFv2 API event",
        tcData ? tcData.eventStatus : "no event data",
        "success:",
        success,
        "data",
        tcData
...
fg_api.prototype.click2playModule = function () {
  ...
  module.prototype.createOverlay = function () {
    ...
    if (M.options.showTeaser) {
      if (!M.status.backgroundLoaded) {
        ...
        else M.status.backgroundLoaded = Promise.resolve();
      }
      M.status.backgroundLoaded.then(function () {
        M.overlayElements.branding.style.background =
          '#000 url("' +
    ...
    M.overlayElements.privacyInfo.innerHTML =
    ...
    M.overlayElements.playBtnContainer.appendChild(
      M.overlayElements.playBtnStage
    );
    ...
    if (
      M.options.showPolicy &&
      !(self.consent && self.consent.isPlatformLoaded())
    ) {
      ...
      if (self.adapters) {
        self.adapters.add("consent", "platformLoaded", function () {
          if (!M.overlayElements.stageFooter) return;
  ...
  module.prototype.showOverlay = function (
    ...
    Promise.race([
      self.wait(M.MAX_WAIT_FOR_LOAD_MS),
      Promise.all([overlayReady, M.documentReady()]),
    ])
      .then(function () {
      ...
      .then(function () {
        ...
        if (self.navigation) self.navigation.invisible();
...
if (
  ...
  typeof window["GameInterface"] === "undefined"
) {
  window["GameInterface"] = (function () {
    let GAME_ID = "";
    const IS_PROD = true;
    const VERSION = "1.1.1";
    ...
    const BRIDGE = {
      ...
      famobi: {
        ...
        sendScore(score) {
          this.score = score;
          return window.famobi_analytics.trackEvent("EVENT_LIVESCORE", {
        ...
        gameStart(level = "") {
          ...
          this.level = level;
          return window.famobi_analytics.trackEvent(
            restarted ? "EVENT_LEVELRESTART" : "EVENT_LEVELSTART",
        ...
        gameOver() {
          this.result = "fail";
          return window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
            eventName: "LEVELEND",
            score: this.score || 0,
            result: this.result,
          });
        ...
        gameQuit() {
          return window.famobi_analytics.trackEvent("EVENT_LEVELFAIL", {
            levelName: "" + this.level,
            reason: "quit",
          });
        },
        ...
        isInterstitialAdAvailable(eventId) {
          return (
            window.famobi.ads.isEnabled() &&
            window.famobi.ads.hasCooledDown() &&
            this.hasCooledDown()
          );
        },
          ...
          if (window.famobi.hasFeature("rewarded")) {
            setInterval(() => {
              if (hasRewardedAd !== window.famobi.hasRewardedAd()) {
                ...
                if (
                  typeof CALLBACKS.onRewardedAdAvailabilityChange === "function"
                ) {
    ...
    const GAME = {
      ...
      mute(isMuted = true, isAdTriggered = false) {
        ...
        if (
          this.muted !== muted &&
          typeof this.callbacks.onAudioStateChange === "function"
        ) {
      ...
      pause(isPaused = true, isAdTriggered = false) {
        ...
        if (
          this.paused !== paused &&
          typeof this.callbacks.onPauseStateChange === "function"
        ) {
...
fg_api.prototype.iapModule = function () {
  ...
  function module() {
    this.log =
      ...
      ["1", "true"].includes(famobi.getUrlParams()["fg_test"]);
    this.promise = null;
    this.gameId = null;
    this.uid = null;
      ...
      "words-of-wonders": [
        {
          ...
          imageURI:
            "https://cdn3.xsolla.com/img/misc/images/40df85f4afeb1a825f9b85c321b127a1.png",
          displayPrice: "1.99",
          priceValue: 1.99,
          priceCurrencyCode: "USD",
          currencyImageURI:
            "https://yastatic.net/s3/games-static/static-data/images/payments/sdk/yan/currency-icon-m.png",
        },
        {
          ...
          imageURI:
            "https://cdn3.xsolla.com/img/misc/images/1f1660e0fcdc1d6ef693624c3bf5fc39.png",
        ...
        },
        {
          ...
          imageURI:
            "https://cdn3.xsolla.com/img/misc/images/77875c8e50fde93803bef0fe0ec7a602.png",
        ...
        },
        {
          ...
          imageURI:
            "https://cdn3.xsolla.com/img/misc/images/c3d0138759ce5669b9e86bcdebec253b.png",
          displayPrice: "19.99",
          priceValue: 19.99,
          priceCurrencyCode: "USD",
          currencyImageURI:
        ...
        },
        {
          ...
          imageURI:
            "https://cdn3.xsolla.com/img/misc/images/34df917aa796b518041ba9e2fd24199f.png",
        ...
        },
        {
          ...
          imageURI:
            "https://cdn3.xsolla.com/img/misc/images/fe88e3492e32521246eb96fac5ae1bc7.png",
  ...
  module.prototype.initialize = function (gameId, uid) {
    ...
    if (!self.hasFeature("iap")) {
      M.log &&
        console.warn(
          "[IAP] initializing... Aborted! [feature 'iap' is disabled]"
        );
      return Promise.resolve();
    }
  ...
  module.prototype.getProducts = function () {
    return new Promise((resolve) => {
      if (!M.isInitialized) {
        M.log &&
          console.warn(
            "[IAP] Getting products... Aborted! [Not initialized yet]"
          );
        return resolve([]);
      }
  ...
  module.prototype.buyProduct = function (sku) {
    return new Promise((resolve) => {
      ...
      if (!M.isInitialized) {
        M.log &&
          console.warn(
            "[IAP] Buying product... Aborted! [Not initialized yet]"
          );
      ...
      if (
        !(
          M.gameId in M.products &&
          M.products[M.gameId].some((product) => product.sku === sku)
        )
      ) {
        M.log &&
          console.warn(
            "[IAP] Buying product... Aborted! [SKU '%s' not found]",
            sku
          );
        return resolve(result);
      }
      ...
      if (Math.random() > 0.9) {
        result.error =
          "This is an intentional error injected for testing. It is triggered randomly with a ~10% probability.";
      }
  ...
  module.prototype.consumeProduct = function (purchase) {
    return new Promise((resolve, reject) => {
      ...
      if (!M.isInitialized) {
        return reject(
          "[IAP] Consuming product... Aborted! [Not initialized yet]"
        );
      }
      ...
      if (
        !(
          ...
          typeof purchase.hash === "string" &&
          typeof purchase.sku === "string"
        )
      ) {
        return reject(
          "[IAP] Consuming product... Aborted! [Invalid param 'purchase']",
          purchase
        );
      }
      ...
      if (
        !(
          M.gameId in M.products &&
          M.products[M.gameId].some((product) => product.sku === purchase.sku)
        )
      ) {
        return reject(
          "[IAP] Consuming product... Aborted! [Invalid param 'purchase']",
...
fg_api.prototype.trackingModule = function () {
  ...
  module.prototype.initializeGoogleTracking = function () {
    ...
    window.famobi_gtag("event", "famobi_api_init", {
      package_id: window.famobi_gameID,
    ...
    trackEvent: function (event, params) {
      return new Promise(function (resolve, reject) {
        ...
        var paramsValidator = function () {
          ...
          typeof famobi.gameanalytics !== "undefined" &&
            famobi.gameanalytics.trackEvent(event, params);
...
fg_api.prototype.gameanalyticsModule = function () {
  ...
  function module() {
    ...
    this.setEnabledInfoLog =
      self.getUrlParams().fg_env === "staging" || this.fg_test;
    this.setEnabledEventSubmission = true;

    self.config.gameParams.gameanalytics =
      self.config.gameParams.gameanalytics || {};
    ...
    this.skipFamobiAnalytics =
      self.config.gameParams.gameanalytics.skip_famobi_analytics || false;

    this.force = !!self.config.gameParams.gameanalytics.force;

    let build = self.config.gameParams.gameanalytics.build;
    this.build =
      typeof build === "string" && build.length ? build : "famobi-web 1.0.0";
  ...
  module.prototype.trackEvent = function (event, params) {
    ...
    if (
      !self.hasFeature("gameanalytics") ||
      typeof window.gameanalytics.GameAnalytics === "undefined"
    ) {
      return;
    }
    ...
    switch (event) {
      case "EVENT_LEVELFAIL":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_LEVELFAIL:" + params.reason,
        });
        break;
      case "EVENT_LEVELRESTART":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_LEVELRESTART",
        });
        break;
      case "EVENT_LEVELSTART":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_LEVELSTART",
        });
        break;
      case "EVENT_LEVELSUCCESS":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_LEVELSUCCESS",
        });
        break;
      case "EVENT_LEVELSCORE":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_LEVELSCORE",
          value: params.levelScore,
        });
        break;
      ...
      case "EVENT_TOTALSCORE":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_TOTALSCORE",
      ...
      case "EVENT_VOLUMECHANGE":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_VOLUMECHANGE",
        });
        break;
      case "EVENT_CUSTOM":
        ...
        if (params.eventName === "LEVELEND") {
          window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
            eventName: "ga:design",
            eventId: "TrackEvent:LEVELEND:" + params.result,
            value: params.score,
          });
        }
        break;
      case "EVENT_TUTORIALCOMPLETED":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_TUTORIALCOMPLETED",
        });
        break;
      case "EVENT_TUTORIALSKIPPED":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_TUTORIALSKIPPED",
        });
        break;
      case "EVENT_PAUSE":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_PAUSE",
        });
        break;
      case "EVENT_RESUME":
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
          eventName: "ga:design",
          eventId: "TrackEvent:EVENT_RESUME",
        });
        break;
      default:
      // do nothing
  ...
  module.prototype.initialize = function (params) {
    ...
    M.promise = new Promise((resolve, reject) => {
      if (!self.hasFeature("gameanalytics") && !M.force) {
        return reject(
          "[GameAnalytics] GA is not supported (feature 'gameanalytics' is disabled)"
        );
      }
      ...
      if (
        !(
          typeof M.game_key === "string" &&
          M.game_key.length &&
          typeof M.secret_key === "string" &&
          M.secret_key.length
        )
      ) {
        return reject(
          "[GameAnalytics] GA is not supported (missing or invalid keys)"
        );
      }

      M.build =
        typeof params.build === "string" && params.build.length
          ? params.build
          : M.build;

      const setEnabledInfoLog =
        typeof params.setEnabledInfoLog === "boolean"
          ? params.setEnabledInfoLog
          : M.setEnabledInfoLog;
      const setEnabledEventSubmission =
        typeof params.setEnabledEventSubmission === "boolean"
          ? params.setEnabledEventSubmission
          : M.setEnabledEventSubmission;

      // available items
      const resource_currencies =
        params.game_analytics.resource_currencies ||
        self.config.game_analytics.resource_currencies ||
        null;
      const resource_item_types =
        params.game_analytics.resource_item_types ||
        self.config.game_analytics.resource_item_types ||
        null;
      const custom_01 =
        params.game_analytics.custom_01 ||
        self.config.game_analytics.custom_01 ||
        null;
      const custom_02 =
        params.game_analytics.custom_02 ||
        self.config.game_analytics.custom_02 ||
        null;
      const custom_03 =
        params.game_analytics.custom_03 ||
        self.config.game_analytics.custom_03 ||
        null;

      const onScriptLoaded = () => {
        if (typeof GameAnalytics !== "undefined") {
          M.log &&
            console.log("[GameAnalytics] Initializing...", {
              game_key: M.game_key,
              build: M.build,
              force: M.force,
              setEnabledInfoLog,
          ...
          custom_01 &&
            GameAnalytics("configureAvailableCustomDimensions01", custom_01);
          custom_02 &&
            GameAnalytics("configureAvailableCustomDimensions02", custom_02);
          custom_03 &&
            GameAnalytics("configureAvailableCustomDimensions03", custom_03);
...
fg_api.prototype.mixpanelModule = function () {
  ...
  module.prototype.trackEvent = function (event, params) {
    ...
    if (
      params.event === "EVENT_LIVESCORE" ||
      (params.event === "EVENT_CUSTOM" &&
        params.eventName.toLowerCase().startsWith("ga:"))
    ) {
      return;
    }
    ...
    M.log &&
      console.log("mixpanel.track(%s, %s)", event, JSON.stringify(params));
  ...
  module.prototype.initialize = function (params) {
    ...
    M.promise = new Promise((resolve, reject) => {
      if (!self.hasFeature("mixpanel") && !M.force) {
        return reject(
          "[mixpanel] Mixpanel is not supported (feature 'mixpanel' is disabled)"
        );
      }
...
if (
  typeof window !== "undefined" &&
  typeof fenster === "undefined" &&
  typeof window.fenster === "undefined"
) {
  !(function (a, b) {
    const fenster = (function () {
      ...
      function update() {
        if (
          typeof window.famobi === "undefined" ||
          typeof window.famobi.getOffsets !== "function"
        ) {
          return;
        }
        ...
        isDebug &&
          console.log(
            "[offsets] innerWidth: %s, innerHeight: %s",
            innerWidth,
            innerHeight
          );
