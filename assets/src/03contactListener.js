...
function onContact(data) {
  ...
  if (collisionData.collisionType == "ball") {
    ...
    if (gameInfo.trial == false) {
      //play ball sound
      var closingSpeed = collisionObject.velocity.minus(
        collisionObject.targetVelocity
      ).magnitude;
  ...
  if (
    collisionData.collisionType == "line" ||
    collisionData.collisionType == "vertex"
  ) {
...
function awardBonuses(collisionData) {
  ...
  if (collisionData.ball.id != 0) {
    ...
    if (projectInfo.mode == 1 && gameInfo.turn == "p1") {
      createBonusText(
        0,
        String(gameInfo.multiplier * 10),
        "font6",
        pocket.dropPosition.x * gameInfo.physScale,
        pocket.dropPosition.y * gameInfo.physScale,
        56,
        false
      );
...
function playPocketAnimation(collisionData) {
  ...
  if (gameInfo.trial == false) {
    ...
    if (ball.id != 0) {
      //if(1==0){
      pocketTween2.onComplete.add(function () {
        removeMC(ball);
      }, this);
    } else {
      pocketTween2.onComplete.add(function () {
