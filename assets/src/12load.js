...
loadState.init = function () {
  ...
  loaderInfo.loaderHighlight = new Phaser.Sprite(
    game,
    0,
    80,
    "loaderHighlight"
  );
  loaderInfo.loaderHighlight.anchor = new Point(0.5, 0);
  ...
  this.resizeGame();
  var tween = game.add.tween(loaderInfo.loaderCanvas).to(
    {
      alpha: 1,
    },
    1000,
...
loadState.resizeGame = function (scale, bounds) {
  ...
  var margin =
    String(
      Math.abs(clientHeight - document.getElementById("mygame").clientHeight) /
        2
    ) + "px";
  ...
  loaderInfo.graphics.drawRect(
    game.width / 2 - 375 / 2,
    game.height / 2 + loaderInfo.loaderBase.y + 8,
    loaderInfo.loaderProgress * 3.75,
    30
  );
};
...
loadState.preload = function () {
  ...
  this.load.image(
    "panel4",
    externalMute ? "assets/img/panel4e.png" : "assets/img/panel4d.png"
  );
