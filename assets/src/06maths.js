  ...
  if (discriminant < 0) {
    ...
    if (t1 >= 0 && t1 <= 1) {
      ...
      result.enter = new Point(
        Maths.fixNumber(result.enter.x),
        Maths.fixNumber(result.enter.y)
      );
    ...
    if (t2 >= 0 && t2 <= 1) {
      ...
      result.exit = new Point(
        Maths.fixNumber(result.exit.x),
        Maths.fixNumber(result.exit.y)
      );
      result.intersects = true;
      return result;
    }
...
Maths.fixNumber = function (numberValue) {
  ...
  return isNaN(Number(numberValue))
    ? 0
    : Math.round(Number(numberValue) * 10000) / 10000;
  //return numberValue;
};
