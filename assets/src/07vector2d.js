...
Vector2D.prototype = {
  ...
  set angle(newAngle) {
    ...
    var currentMagnitude = Math.sqrt(
      Math.pow(this.xValue, 2) + Math.pow(this.yValue, 2)
    );
...
Vector2D.prototype.rotate = function (rotationAngle) {
  ...
  var currentMagnitude = Math.sqrt(
    Math.pow(v.xValue, 2) + Math.pow(v.yValue, 2)
  );
  var newAngleRadians =
    (Math.atan2(v.yValue, v.xValue) * (180 / Math.PI) + Number(rotationAngle)) *
    (Math.PI / 180);
...
Vector2D.prototype.cross = function (v) {
  if (v instanceof Vector2D) {
    return Math.abs(
      Maths.fixNumber(this.xValue * v.yValue - this.yValue * v.xValue)
    );
  }
  return 0;
};
...
Vector2D.prototype.angleBetween = function (v) {
  if (v instanceof Vector2D) {
    return Maths.fixNumber(
      Math.acos(this.dot(v) / (this.magnitude * v.magnitude)) * (180 / Math.PI)
    );
  }
  return 0;
};
