  ...
  do {
    ...
    for (var b = 0; b < numBalls; b++) {
      ...
      if (ball.active == true && skipBall == false) {
        var nextBallPosition = ball.position.plus(
          ball.velocity.times(omegaTime)
        ); //position of ball at start of next frame assuming no collisions
        ...
        for (var t = s; t < this.ballArray.length; t++) {
          //t = b prevents double checks like 1:2 and 2:1. t = 0 checks all balls
          var target = this.ballArray[t];

          if (
            ball.velocity.magnitudeSquared != 0 ||
            target.velocity.magnitudeSquared != 0
          ) {
            // balls can't collide if neither are moving.  However, keeping this in won't tell us if balls are touching (if that is actually possible - not sure).
            ...
            if (target != ball && target.active == true && close == true) {
              //ball cannot collide with itself
              ...
              if (
                ...
                var nextBallReferencePoint = ball.position.plus(
                  ballReferenceVector.times(omegaTime)
                );

                var A = new Point(ball.position.x, ball.position.y); //starting point of object ball
                var B = new Point(
                  nextBallReferencePoint.x,
                  nextBallReferencePoint.y
                ); //point of object ball at next frame
                ...
                if (
                  ...
                  if (intersection.intersects == true) {
                    intersectPointAsVector2 = new Vector2D(
                      intersectPoint2.x,
                      intersectPoint2.y
                    );
                    var vectorAB2 = Maths.createVectorFrom2Points(A, B); //current ball position to ball position at next frame in target's f.o.r.
                    var vectorAP2 = Maths.createVectorFrom2Points(
                      A,
                      intersectPoint2
                    ); //current ball position to intersect point
                    intersectTime2 = Maths.fixNumber(
                      time +
                        (vectorAP2.magnitude / vectorAB2.magnitude) * omegaTime
                    ); //time of collision since start of frame.  Value is between 0 and 1, where 0 is the time at the beginning of the frame and 1 is the end of the frame
                  }
                  ...
                  if (intersection.inside == true) {
                    ...
                    var targetToBall = ball.position
                      .minus(target.position)
                      .normalize();
                    intersectPointAsVector2 = target.position.plus(
                      targetToBall.times(r)
                    );
                    intersectTime2 = time;
                  }
                  ...
                  if (intersectTime2 < collisionTime) {
                    ...
                    if (intersection.intersects == true) {
                      ...
                      collision.objectIntersectPoint = ball.position.plus(
                        ball.velocity.times(collisionTime - time)
                      );

                      //use same process to calculate target's collision point
                      collision.targetIntersectPoint = target.position.plus(
                        target.velocity.times(collisionTime - time)
                      );
                    ...
                    if (intersectTime2 != 1) {
                      ...
                      if (intersection.inside == true) {
                        collision.objectIntersectPoint =
                          intersectPointAsVector2;
                        collision.targetIntersectPoint = target.position;
                      }
        ...
        if (ball.velocity.magnitudeSquared != 0) {
          ...
          for (var l = 0; l < this.lineArray.length; l++) {
            ...
            var path = new Phaser.Line(
              ball.position.x,
              ball.position.y,
              nextBallPosition.x,
              nextBallPosition.y
            );
            var intersectPoint = path.intersects(
              new Phaser.Line(line.p3.x, line.p3.y, line.p4.x, line.p4.y)
            );

            //NEW!! if no intersection is found, double check against secondary line, slightly behind the first.

            if (intersectPoint == null) {
              intersectPoint = Maths.lineIntersectLine(
                new Point(ball.position.x, ball.position.y),
                new Point(nextBallPosition.x, nextBallPosition.y),
              ...
              if (intersectPoint != null) {
                ...
                var currentPointAsVec = new Vector2D(
                  intersectPoint.x,
                  intersectPoint.y
                );
            ...
            if (intersectPoint != null) {
              var intersectPointAsVector = new Vector2D(
                intersectPoint.x,
                intersectPoint.y
              );
              //collisionDetected = true;
              ...
              var vectorAB = Maths.createVectorFrom2Points(
                ball.position,
                nextBallPosition
              ); //current ball position to ball position at next frame
              var vectorAP = Maths.createVectorFrom2Points(
                ball.position,
                intersectPointAsVector
              ); //current ball position to intersect point
              var intersectTime = Maths.fixNumber(
                time + (vectorAP.magnitude / vectorAB.magnitude) * omegaTime
              );
              //trace("time: " + time);
          ...
          for (var v = 0; v < this.vertexArray.length; v++) {
            ...
            if (close == true) {
              ...
              if (
                intersection3.intersects == true ||
                intersection3.inside == true
              ) {
                ...
                if (intersection3.intersects == true) {
                  intersectPointAsVector3 = new Vector2D(
                    intersectPoint3.x,
                    intersectPoint3.y
                  );
                  var vectorAB3 = Maths.createVectorFrom2Points(A3, B3); //current ball position to ball position at next frame
                  var vectorAP3 = Maths.createVectorFrom2Points(
                    A3,
                    intersectPoint3
                  ); //current ball position to intersect point
                  intersectTime3 = Maths.fixNumber(
                    time +
                      (vectorAP3.magnitude / vectorAB3.magnitude) * omegaTime
                  );
                }
                ...
                if (intersection3.inside == true) {
                  ...
                  var A3PV = ball.position.plus(
                    ball.velocity.normalize().times(-r3 * 2)
                  ); //position of ball A3, projected backwards along the line of it's own velocity, as a vector, long enough to exit the vertex circle
                  var A3P = new Point(A3PV.x, A3PV.y);
                  var intersectionCorrection = Maths.lineIntersectCircle(
                    A3,
                    A3P,
                    C3,
                    r3
                ...
                if (intersectTime3 < collisionTime) {
                  ...
                  if (intersectTime3 != 1) {
                    ...
                    if (intersection3.intersects == true) {
                      collision.objectIntersectPoint = new Vector2D(
                        intersectPoint3.x,
                        intersectPoint3.y
                      );
                    }
          ...
          for (var p = 0; p < this.pocketArray.length; p++) {
            ...
            if (towardsPocket == true && close == true) {
              ...
              if (
                intersection4.intersects == true ||
                intersection4.inside == true
              ) {
                ...
                if (intersection4.intersects == true) {
                  intersectPointAsVector4 = new Vector2D(
                    intersectPoint4.x,
                    intersectPoint4.y
                  );
                  var vectorAB4 = Maths.createVectorFrom2Points(A4, B4); //current ball position to ball position at next frame
                  var vectorAP4 = Maths.createVectorFrom2Points(
                    A4,
                    intersectPoint4
                  ); //current ball position to intersect point
                  intersectTime4 = Maths.fixNumber(
                    time +
                      (vectorAP4.magnitude / vectorAB4.magnitude) * omegaTime
                  );
                }
                ...
                if (intersection4.inside == true) {
                  ...
                  var vertexToBall = Maths.createVectorFrom2Points(
                    C4,
                    A4
                  ).normalize();
                  intersectPointAsVector4 = new Vector2D(C4.x, C4.y).plus(
                    vertexToBall.times(r4)
                  );
                  intersectTime4 = time;
...
*/
      ...
      var ballVelocityNormal = contactNormal.times(
        ball.velocity.dot(contactNormal)
      );
      var ballVelocityTangent = contactTangent.times(
        ball.velocity.dot(contactTangent)
      );
      //same for target ball
      var targetVelocityNormal = contactNormal.times(
        target.velocity.dot(contactNormal)
      );
      var targetVelocityTangent = contactTangent.times(
        target.velocity.dot(contactTangent)
      );
      ...
      var newNormalVelocityBall = targetVelocityNormal
        .times(this.ballRestitution)
        .plus(ballVelocityNormal.times(1 - this.ballRestitution));

      //=======================================

      var newNormalVelocityTarget = ballVelocityNormal
        .times(this.ballRestitution)
        .plus(targetVelocityNormal.times(1 - this.ballRestitution));
    ...
    if (collision.type == "line") {
      ...
      var ballVelocityNormal = line.normal.times(
        ball.velocity.dot(line.normal)
      );
      var ballVelocityTangent = line.direction.times(
        ball.velocity.dot(line.direction)
      );

      //apply english.
      if (ball.id == 0) {
        ballVelocityTangent = ballVelocityTangent.plus(
          line.direction.times(
            Maths.fixNumber(ball.english * 0.2 * ball.velocity.magnitude)
          )
      ...
      ball.velocity = ballVelocityNormal
        .times(-this.cushionRestitution)
        .plus(ballVelocityTangent);
...
billiardPhysics.prototype.moveBalls = function (delta) {
  for (var n = 0; n < this.ballArray.length; n++) {
    ...
    if (
      ...
      this.omissionArray.indexOf(ball) == -1
    ) {
      if (ball.active == true) {
...
billiardPhysics.prototype.updateFriction = function () {
  ...
  for (var b = 0; b < this.ballArray.length; b++) {
    ...
    if (ball.ySpin != 0) {
      ...
      var swerve = normal.times(
        (0.3 * ball.ySpin * ball.velocity.magnitude) / 800
      );
